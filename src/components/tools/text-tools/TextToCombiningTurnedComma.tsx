...
export default function TextToCombiningTurnedComma({ toolId, toolName }: { toolId: string; toolName: string }) {
  ...
  const transform = () => {
    ...
    const combiningTurnedCommaAbove = '\u0312';
    const result = input
      .split('')
      .map((char) => {
        if (lettersOnly && !/[a-zA-Z]/.test(char)) return char;
        return char + combiningTurnedCommaAbove;
      })
      .join('');
    ...
    <div className="space-y-4" data-tool-id={toolId}>
      <InputArea>
        ...
        <textarea
          ...
          placeholder="Enter text to add combining turned comma above..."
