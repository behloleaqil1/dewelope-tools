...
export default function KeepOnlyLetters({ toolId, toolName }: { toolId: string; toolName: string }) {
  ...
  return (
    <div className="space-y-4" data-tool-id={toolId}>
      <InputArea>
        ...
        <textarea id={`${toolId}-input`} value={input} onChange={(e) => setInput(e.target.value)} placeholder="Hello123 World!@#" aria-label={`Input for ${toolName}`} className="input-field h-28 resize-y" />
      </InputArea>
