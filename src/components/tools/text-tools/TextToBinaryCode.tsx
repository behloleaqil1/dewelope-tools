...
export default function TextToBinaryCode({ toolId, toolName }: { toolId: string; toolName: string }) {
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(() => {
      setOutput([...input].map(c => c.charCodeAt(0).toString(2).padStart(8, '0')).join(' '));
    }, 200);
