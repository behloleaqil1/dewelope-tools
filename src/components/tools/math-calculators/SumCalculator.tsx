...
/**
 * SumCalculator - Sum a list of numbers with running total display.
...
export default function SumCalculator({ toolId, toolName }: { toolId: string; toolName: string }) {
  ...
  const [result, setResult] = useState<{ numbers: number[]; runningTotals: number[]; total: number } | null>(null);
  ...
  const calculate = () => {
    ...
    setResult({ numbers, runningTotals, total: sum });
  };

  const copyText = result
    ? `Numbers: ${result.numbers.join(', ')}\nSum: ${result.total}\nRunning totals: ${result.runningTotals.join(', ')}`
    : '';
        ...
        {result && (
          <div className="space-y-3">
            <div className="bg-gray-50 p-4 rounded-lg border border-gray-200 text-center">
              ...
              <div className="text-xs text-gray-500 mt-1">Total Sum ({result.numbers.length} numbers)</div>
            </div>
            <div className="bg-gray-50 p-3 rounded-lg border border-gray-200">
              <div className="text-sm font-medium text-gray-700 mb-2">Running Total</div>
              <div className="space-y-1">
                {result.numbers.map((num, i) => (
                  <div key={i} className="flex justify-between text-sm font-mono text-gray-700">
                    <span>{i > 0 ? `+ ${num}` : num}</span>
                    <span className="text-gray-500">= {result.runningTotals[i]}</span>
                  </div>
                ))}
              </div>
            </div>
