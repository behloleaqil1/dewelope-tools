...
export default function MeanMedianMode({ toolId, toolName }: { toolId: string; toolName: string }) {
  const [input, setInput] = useState('');
  const [output, setOutput] = useState('');

  const calculate = () => {
    const nums = input.split(/[,\s]+/).map(Number).filter(n => !isNaN(n));
    if (nums.length === 0) { setOutput('Please enter numbers separated by commas or spaces.'); return; }
    ...
    const modeStr = modes.length === 0 ? 'No mode (all values appear once)' : modes.join(', ');
    setOutput(`Dataset: ${sorted.join(', ')}\nCount: ${nums.length}\n\nMean: ${mean.toFixed(4)}\nMedian: ${median}\nMode: ${modeStr}\nRange: ${sorted[sorted.length - 1] - sorted[0]}\nSum: ${nums.reduce((a, b) => a + b, 0)}`);
  };
