...
/**
 * ColorBrightnessAdjuster - Lighten or darken any color by percentage.
 */
export default function ColorBrightnessAdjuster({ toolId, toolName }: { toolId: string; toolName: string }) {
  ...
  const adjustedL = Math.min(100, Math.max(0, Math.round(l * brightness / 100)));
  const adjustedColor = hslToHex(h, s, adjustedL);

  // Generate brightness scale
  const scale = [0, 25, 50, 75, 100, 125, 150, 175, 200].map(pct => ({
    pct,
    color: hslToHex(h, s, Math.min(100, Math.max(0, Math.round(l * pct / 100)))),
  }));

  const copyText = `Original: ${color} (HSL: ${h}°, ${s}%, ${l}%)\nAdjusted: ${adjustedColor} (HSL: ${h}°, ${s}%, ${adjustedL}%)\nRGB: ${hexToRgb(adjustedColor)}\nBrightness: ${brightness}%`;
      ...
      <InputArea>
        ...
        <div className="mt-4">
          <label className="block text-sm font-medium text-gray-700 mb-1">
            Brightness: {brightness}% {brightness < 100 ? '(darker)' : brightness > 100 ? '(lighter)' : '(original)'}
          </label>
          <input
            type="range"
            min="0"
      ...
      <OutputArea hasContent={true}>
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="bg-gray-50 p-3 rounded-lg border border-gray-200 text-center">
              <div className="text-lg font-bold font-mono text-gray-800">{hexToRgb(adjustedColor)}</div>
              <div className="text-xs text-gray-500">Adjusted RGB</div>
            </div>
          </div>

          <div className="bg-gray-50 p-3 rounded-lg border border-gray-200">
            <div className="text-sm font-medium text-gray-700 mb-2">Brightness Scale</div>
            <div className="flex h-8 rounded overflow-hidden">
              {scale.map((item, i) => (
                <div key={i} className="flex-1" style={{ backgroundColor: item.color }} title={`${item.pct}%`} />
              ))}
            </div>
