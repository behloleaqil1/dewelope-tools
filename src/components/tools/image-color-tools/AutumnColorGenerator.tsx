...
/**
 * AutumnColorGenerator - Generate autumn/fall-themed color palettes.
 */
export default function AutumnColorGenerator({ toolId, toolName }: { toolId: string; toolName: string }) {
  const [count, setCount] = useState('6');
  const [colors, setColors] = useState<string[]>([]);

  const autumnHues = [
    { h: [10, 30], s: [60, 85], l: [35, 50] },    // burnt orange
    { h: [0, 15], s: [50, 75], l: [30, 45] },     // deep red
    { h: [35, 50], s: [70, 90], l: [45, 60] },    // golden yellow
    { h: [20, 40], s: [40, 65], l: [25, 40] },    // warm brown
    { h: [45, 60], s: [50, 75], l: [50, 65] },    // harvest gold
    { h: [5, 20], s: [55, 80], l: [40, 55] },     // rust
    { h: [25, 45], s: [30, 55], l: [20, 35] },    // dark amber
    { h: [350, 360], s: [40, 65], l: [30, 45] },  // cranberry
  ];
  ...
  const generate = () => {
    ...
    for (let i = 0; i < n; i++) {
      const preset = autumnHues[i % autumnHues.length];
  ...
  return (
    <div className="space-y-4" data-tool-id={toolId}>
      ...
      <button onClick={generate} className="btn-primary" aria-label="Generate autumn palette">Generate Autumn Palette</button>

      <OutputArea hasContent={colors.length > 0}>
        {colors.length > 0 && (
          <div className="space-y-3">
            <label className="block text-sm font-medium text-gray-700">Autumn Palette</label>
