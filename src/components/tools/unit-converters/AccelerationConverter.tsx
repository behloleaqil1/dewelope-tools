...
/**
 * AccelerationConverter - Convert between m/s², ft/s², g-force, gal.
 */
export default function AccelerationConverter({ toolId, toolName }: { toolId: string; toolName: string }) {
  const [value, setValue] = useState('');
  const [fromUnit, setFromUnit] = useState('ms2');
  ...
  const TO_MS2: Record<string, number> = {
    ms2: 1,
    fts2: 0.3048,
    g: 9.80665,
    gal: 0.01,
  ...
  function convert() {
    ...
    const ms2 = num * TO_MS2[fromUnit];
    const converted = ms2 / TO_MS2[toUnit];

    setResult(converted.toLocaleString(undefined, { maximumFractionDigits: 8 }));

    const all = UNITS.map((u) => ({
      unit: u.label,
      value: (ms2 / TO_MS2[u.value]).toLocaleString(undefined, { maximumFractionDigits: 8 }),
    }));
    setAllResults(all);
  }
    ...
    <div className="space-y-4" data-tool-id={toolId}>
      <InputArea error={error}>
        <label htmlFor={`${toolId}-value`} className="block text-sm font-medium text-gray-700 mb-1">
          Acceleration value for {toolName}
        </label>
        <input
          id={`${toolId}-value`}
          type="text"
          ...
          aria-label="Acceleration value to convert"
        ...
        <div className="grid grid-cols-2 gap-3 mt-3">
          <div>
            ...
            <select
              ...
              aria-label="Source acceleration unit"
          ...
          <div>
            ...
            <select
              ...
              aria-label="Target acceleration unit"
              className="input-field text-sm"
            >
              {UNITS.map((u) => (
                <option key={u.value} value={u.value}>{u.label}</option>
      ...
      <button onClick={convert} aria-label="Convert acceleration" className="btn-primary">
        Convert
      </button>
