...
/**
 * ElectricCurrentConverter - Converts between electric current units: Ampere, mA, µA, kA.
 */
export default function ElectricCurrentConverter({ toolId, toolName }: { toolId: string; toolName: string }) {
  ...
  const TO_AMPERE: Record<string, number> = {
    megaampere: 1e6,
    kiloampere: 1e3,
    ampere: 1,
    milliampere: 1e-3,
  ...
  const UNITS = [
    { value: 'megaampere', label: 'Megaampere (MA)' },
    ...
    { value: 'nanoampere', label: 'Nanoampere (nA)' },
    { value: 'picoampere', label: 'Picoampere (pA)' },
  ];
  ...
  function convert() {
    ...
    const amperes = num * TO_AMPERE[fromUnit];
    const converted = amperes / TO_AMPERE[toUnit];
    ...
    <div className="space-y-4" data-tool-id={toolId}>
      <InputArea error={error}>
        <label htmlFor={`${toolId}-value`} className="block text-sm font-medium text-gray-700 mb-1">
          Electric current value for {toolName}
        </label>
        <input
          id={`${toolId}-value`}
          type="text"
          ...
          aria-label="Electric current value to convert"
        ...
        <div className="grid grid-cols-2 gap-3 mt-3">
          <div>
            ...
            <select
              ...
              aria-label="Source current unit"
          ...
          <div>
            ...
            <select
              ...
              aria-label="Target current unit"
              className="input-field text-sm"
            >
              {UNITS.map((u) => (
                <option key={u.value} value={u.value}>{u.label}</option>
      ...
      <button onClick={convert} aria-label="Convert current" className="btn-primary">
        Convert
      </button>
