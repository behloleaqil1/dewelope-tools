...
/**
 * InductanceConverter - Converts between inductance units: Henry, millihenry, microhenry, nanohenry.
 */
export default function InductanceConverter({ toolId, toolName }: { toolId: string; toolName: string }) {
  const [value, setValue] = useState('');
  const [fromUnit, setFromUnit] = useState('henry');
  const [toUnit, setToUnit] = useState('millihenry');
  const [result, setResult] = useState<string | null>(null);
  const [error, setError] = useState<string | undefined>();

  // Conversion factors to Henry
  const TO_HENRY: Record<string, number> = {
    henry: 1,
    millihenry: 1e-3,
    microhenry: 1e-6,
    nanohenry: 1e-9,
  ...
  const UNITS = [
    { value: 'megahenry', label: 'Megahenry (MH)' },
    { value: 'kilohenry', label: 'Kilohenry (kH)' },
    ...
    { value: 'picohenry', label: 'Picohenry (pH)' },
  ];
  ...
  function convert() {
    ...
    const henrys = num * TO_HENRY[fromUnit];
    const converted = henrys / TO_HENRY[toUnit];
    ...
    <div className="space-y-4" data-tool-id={toolId}>
      <InputArea error={error}>
        ...
        <input
          ...
          aria-label="Inductance value to convert"
        ...
        <div className="grid grid-cols-2 gap-3 mt-3">
          <div>
            ...
            <select
              ...
              aria-label="Source inductance unit"
          ...
          <div>
            ...
            <select
              ...
              aria-label="Target inductance unit"
              className="input-field text-sm"
            >
              {UNITS.map((u) => (
                <option key={u.value} value={u.value}>{u.label}</option>
      ...
      <button onClick={convert} aria-label="Convert inductance" className="btn-primary">
        Convert
      </button>
