...
/**
 * ElectricalConductanceConverter - Convert between Siemens, mS, µS, kS, mho.
 */
export default function ElectricalConductanceConverter({ toolId, toolName }: { toolId: string; toolName: string }) {
  ...
  const toSiemens: Record<string, number> = {
    microsiemens: 1e-6,
    millisiemens: 1e-3,
    siemens: 1,
    kilosiemens: 1e3,
  ...
  const convert = () => {
    ...
    const inSiemens = num * toSiemens[fromUnit];
    const converted = Object.entries(toSiemens).map(([unit, factor]) => ({
      unit: unitLabels[unit],
      value: (inSiemens / factor).toFixed(6),
    }));
    ...
    <div className="space-y-4" data-tool-id={toolId}>
      <div className="grid grid-cols-2 gap-4">
        <InputArea error={error}>
          ...
          <input id={`${toolId}-value`} type="text" inputMode="decimal" value={value} onChange={(e) => setValue(e.target.value)} placeholder="e.g. 0.5" aria-label={`Value for ${toolName}`} className="input-field" />
        </InputArea>
        <div>
      ...
      <button onClick={convert} aria-label="Convert conductance" className="btn-primary">Convert</button>
