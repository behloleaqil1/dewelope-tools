...
/**
 * CrystalStructGenerator - Generate Crystal language struct definitions from JSON input.
 * Automatically infers field types including nested structs, arrays, and nilable values.
 */
export default function CrystalStructGenerator({ toolId, toolName }: { toolId: string; toolName: string }) {
  ...
  function generateStruct(obj: Record<string, unknown>, name: string, structs: string[]): void {
    ...
    for (const [key, value] of Object.entries(obj)) {
      ...
      lines.push(`  property ${fieldName} : ${fieldType}`);
    }
    lines.push('end');
    ...
    <div className="space-y-4" data-tool-id={toolId}>
      <InputArea>
        ...
        <input
          ...
          placeholder="Root"
          aria-label="Crystal struct name"
        ...
        />
        <button onClick={handleGenerate} className="btn-primary mt-2">Generate Crystal Struct</button>
      </InputArea>
      ...
      <OutputArea hasContent={!!output}>
        {output && (
          <div className="space-y-2">
            <label className="block text-sm font-medium text-gray-700">Generated Crystal Struct</label>
