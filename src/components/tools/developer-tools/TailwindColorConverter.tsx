...
/** Default Tailwind CSS color palette (subset of most common colors) */
...
/**
 * TailwindColorConverter - Finds the closest Tailwind CSS color class for a given hex color.
 * Uses Euclidean distance in RGB space to determine the closest match.
 */
export default function TailwindColorConverter({ toolId, toolName }: { toolId: string; toolName: string }) {
  const [hexInput, setHexInput] = useState('');
  const [result, setResult] = useState<{ className: string; hex: string; distance: number } | null>(null);
  ...
  const findClosest = () => {
    ...
    if (!inputRgb) {
      setError('Please enter a valid hex color (e.g., #3b82f6 or #fff)');
      setResult(null);
      return;
    }
    ...
    setResult({ className: closestName, hex: closestHex, distance: minDistance });
  };

  const inputRgb = hexToRgb(hexInput);
  const inputHexNormalized = inputRgb
    ? `#${inputRgb.map((c) => c.toString(16).padStart(2, '0')).join('')}`
    : null;
    ...
    <div className="space-y-6" data-tool-id={toolId}>
      <div className="space-y-4">
        <InputArea error={error}>
          <label htmlFor={`${toolId}-hex`} className="block text-sm font-medium text-gray-700">
            Hex Color
          </label>
          <div className="flex gap-2 items-center">
            ...
            {inputHexNormalized && (
              <div
                ...
                style={{ backgroundColor: inputHexNormalized }}
                aria-label={`Preview of input color ${inputHexNormalized}`}
              />
            )}
          </div>
        </InputArea>

        <button onClick={findClosest} className="btn-primary" aria-label="Find closest Tailwind color">
          Find Closest Match
        </button>
      </div>
      ...
      <OutputArea hasContent={result !== null}>
        {result && (
          <div className="space-y-3">
            <h3 className="text-sm font-medium text-gray-700">Closest Tailwind Color</h3>
            <div className="flex items-center gap-4 p-4 bg-gray-50 rounded-lg">
              <div
                className="w-16 h-16 rounded-lg border border-gray-200 flex-shrink-0"
                style={{ backgroundColor: result.hex }}
                aria-label={`Preview of Tailwind color ${result.className}`}
              />
              <div>
                <div className="text-lg font-semibold text-gray-800">{result.className}</div>
                <div className="text-sm text-gray-600 font-mono">{result.hex}</div>
                <div className="text-xs text-gray-500 mt-1">
                  Distance: {result.distance.toFixed(2)}
                  {result.distance === 0 && <span className="ml-1 text-emerald-600 font-medium">Exact match!</span>}
                </div>
              </div>
            </div>
            <div className="text-sm text-gray-600">
              Use as: <code className="bg-gray-100 px-2 py-0.5 rounded text-indigo-600 font-mono">bg-{result.className}</code> or{' '}
              <code className="bg-gray-100 px-2 py-0.5 rounded text-indigo-600 font-mono">text-{result.className}</code>
            </div>
            <CopyToClipboard text={result.className} />
          </div>
        )}
      </OutputArea>
    </div>
