...
import { htmlEntityEncode, htmlEntityDecode } from '@/lib/developer-tools';

const MAX_LENGTH = 1048576; // 1MB

/**
 * HtmlEntityEncoderDecoder - Encode/decode HTML entities.
 * Preserves input on error, provides copy button for output.
 * Requirements: 6.2, 6.5, 6.6, 6.7
 */
export default function HtmlEntityEncoderDecoder({ toolId }: ToolEngineProps) {
    ...
    <div className="space-y-4">
      ...
      <InputArea error={error}>
        <label htmlFor={`${toolId}-input`} className="block text-sm font-medium text-gray-700 mb-1">
          {mode === 'encode' ? 'Text to encode as HTML entities' : 'HTML entities to decode'}
        </label>
        <textarea
          id={`${toolId}-input`}
          value={input}
          onChange={(e) => setInput(e.target.value)}
          placeholder={mode === 'encode' ? 'Enter text with special characters (e.g. <div class="test">)...' : 'Enter HTML entities to decode (e.g. &lt;div&gt;)...'}
          aria-label={mode === 'encode' ? 'Text input for HTML entity encoding' : 'HTML entity input for decoding'}
