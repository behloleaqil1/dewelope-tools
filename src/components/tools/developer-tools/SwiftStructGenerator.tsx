...
/**
 * SwiftStructGenerator - Generates Swift struct definitions from JSON input.
 * Infers types, handles nested objects and arrays, and produces idiomatic Swift code with Codable conformance.
 */
export default function SwiftStructGenerator({ toolId, toolName }: { toolId: string; toolName: string }) {
  ...
  function generateStruct(obj: Record<string, unknown>, name: string, structs: string[]): void {
    ...
    for (const [key, value] of Object.entries(obj)) {
      ...
      fields.push(`    let ${key}: ${type}${optional}`);
    }
    const structStr = `struct ${name}: Codable {\n${fields.join('\n')}\n}`;
  ...
  return (
    <div className="space-y-4" data-tool-id={toolId}>
      ...
      <button onClick={handleConvert} aria-label="Generate Swift struct" className="btn-primary">
        Generate Struct
      </button>
      ...
      <OutputArea hasContent={!!output}>
        {output && (
          <div className="space-y-2">
            <label className="block text-sm font-medium text-gray-700">Swift Struct</label>
